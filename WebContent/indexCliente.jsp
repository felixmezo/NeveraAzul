...
<body>
	...
	<section id="contenido">
		...
		<c:if test="${not empty user}">
			<label>
				Hola
				<c:out value="${nombreCliente}"></c:out>
				<c:out value="${apellido1}"></c:out>
				<c:out value="${apellido2}"></c:out>
			</label>
			<br>
			<c:if test="${not empty pedidos}">
			<div class="panel panel-danger">
				<div class="panel-heading">Pedidos del usuario <c:out value="${user}"></c:out>.</div>
			...
			<c:if test="${empty pedidos}">
				...
				<p>Dale a "Hacer un Pedido" en el panel de navegación para iniciar un pedido o continuar uno en curso</p>
			</c:if>
			<form action="/indexcliente" method="post">
				<button type="submit" class="btn btn-default" name="crearPedido" value="Crear pedido nuevo o continuar pedido en curso">Crear pedido nuevo o continuar pedido en curso</button>
			</form>
